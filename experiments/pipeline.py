...
def read_data(filename: str) -> pd.DataFrame:
    ...
    X = df.drop(columns=["temp", "device", "ts"]).astype(float)
    y = df["temp"]
    X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.33)
    std_scaler = StandardScaler()
    X_train = std_scaler.fit_transform(X_train)
    X_test = std_scaler.transform(X_test)
    return X_train, X_test, y_train, y_test, std_scaler


@task(name="Hyper-Parameter Tunning")
def hyper_parameter_tunning(X_train, X_val, y_train, y_val):
...
def select_best_model(experiment_name):
    try:
        ...
        if experiment is None:
            raise ValueError(
                f"Experimento con nombre '{experiment_name}' no encontrado."
            )
        ...
        client.transition_model_version_stage(
            name="MyModel", version=model_version.version, stage="Staging"
        )
        ...
        client.set_registered_model_alias(
            name="MyModel", alias="challenger", version=model_version.version
        )

        print(
            f"El mejor modelo fue: {best_run_id} con el RMSE: {best_run['metrics.rmse']}"
        )
...
def update_champion_model(model_name="MyModel"):
    try:
        ...
        if not champion_version:
            raise ValueError(
                "No se encontró la versión del modelo con el alias 'Champion'."
            )
        ...
        if not challenger_version:
            raise ValueError(
                "No se encontró la versión del modelo con el alias 'Challenger'."
            )
        ...
        if challenger_rmse < champion_rmse:
            ...
            client.set_registered_model_alias(
                name=model_name, alias="champion", version=challenger_version.version
            )
            print(
                f"El modelo 'Challenger' se ha convertido en el nuevo 'Champion' con RMSE: {challenger_rmse}"
            )
        else:
            print(
                f"El modelo 'Champion' sigue siendo el mejor con RMSE: {champion_rmse}"
            )
...
def main_flow() -> None:
    ...
    # MLflow settings
    dagshub.init(
        url="https://dagshub.com/ioSoyPato/proyecto-final-ciencia-datos", mlflow=True
    )

    MLFLOW_TRACKING_URI = mlflow.get_tracking_uri()

    mlflow.set_tracking_uri(MLFLOW_TRACKING_URI)
    mlflow.set_experiment(experiment_name="temperature-experiments-prefect")

    # Transform
    X_train, X_val, y_train, y_val, std_scaler = read_data("../data/raw/data.csv")

    # Save preprocessor
    pathlib.Path("models").mkdir(exist_ok=True)
    with open("models/preprocessor.b", "wb") as f_out:
        pickle.dump(std_scaler, f_out)

    # Hyper-parameter Tunning
    best_params = hyper_parameter_tunning(X_train, X_val, y_train, y_val)

    # Train
    train_best_model(X_train, X_val, y_train, y_val, best_params)

    # Select best model as a challenger
    select_best_model("temperature-experiments-prefect")
